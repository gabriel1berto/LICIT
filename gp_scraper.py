#!/usr/bin/env python3
"""
gp_scraper.py — Busca preços de pneus na GP Fácil (gpfacil.com.br)
Uso   : python gp_scraper.py items.json [results.json]
Output: JSON com produtos aprovados por item, ordenados por preço

Autenticação: cookies injetados de gp_cookies.json (reCAPTCHA v3 bloqueia
login direto via Playwright — sessão é exportada do MCP browser).

Para renovar cookies: acesse https://www.gpfacil.com.br no MCP, faça login,
e atualize gp_cookies.json com os valores de document.cookie.

Critérios validados por item:
  min_treadwear — "Tradwear" na tabela de specs (typo do site)
  min_ic        — "Índice de carga" → primeiro número
  min_iv        — "Índice de velocidade" → letra
  construcao    — "Tipo de construção" na tabela (belt material não distinguível
                   de radial/diagonal → obs se requerido)
  min_lonas     — Não publicado na GP → sempre obs se requerido
  padrao_piso   — Verificado pelo nome + "Tipo de Terreno" na tabela

Formato de busca GP: '205/75r16c' (lowercase, sem espaço antes do r)
"""
...
def parse_ic_iv(nome: str):
    """Extrai IC e IV do nome (ex: 'Pirelli P7 205/55R16 91H' → 91, 'H')."""
...
def extract_brand(nome: str) -> str:
    """GP: 'Pneu {Marca} Aro N...' → segunda palavra é a marca."""
...
def scrape_listing(page, medida: str) -> list[dict]:
    """Retorna produtos em estoque com preço, ordenados do mais barato."""
    ...
    raw = page.evaluate(r"""() => {
        ...
        document.querySelectorAll('a[href*="/pneu-"] h3').forEach(h3 => {
            ...
            // Sobe até container do card (tem apenas 1 h3 → garante isolamento)
            ...
            // Preço: primeiro elemento folha que começa com R$
            let priceText = null;
            ...
            // Esgotado: parágrafo "esgotado" no pai do container
...
def process_item(page, cfg: dict) -> list[dict]:
    ...
    for c in candidates:
        ...
        # Construção: GP publica "Tipo de construção" mas distingue Radial/Diagonal,
        # não o material (nylon/poliester). Se requerido, adiciona obs.
        if construcao_req:
            gp_const = specs.get("construcao") or ""
            # Só falha se GP explicitamente contradiz (ex: item quer nylon/poliester
            # mas GP diz "diagonal" — improvável para pneus radiais modernos)
            if not gp_const:
                obs_list.append(
                    f"Construção ({construcao_req}) não publicada na GP — confirmar com fornecedor"
                )
            # Radial: aceito para qualquer construção_req (nylon/poliester são radiais)

        # Número de lonas: GP não publica
        if min_lonas:
            obs_list.append(
...
def main():
    if len(sys.argv) < 2:
        print("uso: python gp_scraper.py items.json [results.json]", file=sys.stderr)
        sys.exit(1)
