#!/usr/bin/env python3
"""
bransales_scraper_master.py — Cópia de bransales_scraper.py adaptada pro fluxo
de COTAÇÃO MASTER (coleta diária de mercado, não ligada a edital específico).

Diferenças vs. bransales_scraper.py (original, NUNCA alterar, segue servindo
o fluxo de cotação por edital):
  ...
  - Ficha técnica sempre visitada (need_page fixo em True) — original só
    visita quando algum critério de filtro (min_treadwear/construcao/
    min_lonas) é exigido; cotação master não tem critério, mas precisa do
    dado técnico completo pra ser utilizável sem reabrir o site manualmente.
  - Delay entre visita de ficha técnica (rate-limit, reduz risco de WAF —
    Bransales já retornou 403 pra request sem browser real).

Uso   : python bransales_scraper_master.py items.json [results.json]
...
DELAY_FICHA = 1.5  # segundos entre visita de ficha técnica — rate-limit
...
def process_item(page, cfg: dict) -> list[dict]:
    ...
    for c in candidates:
        ...
        # cotação master: SEMPRE visita a ficha (original só visita se houver
        # critério de filtro — aqui o objetivo é dado completo e utilizável)
...
def main():
    if len(sys.argv) < 2:
        print("uso: python bransales_scraper_master.py items.json [results.json]", file=sys.stderr)
        sys.exit(1)
